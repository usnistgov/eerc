...
const addPrices = (prices, carbon, carbonprice, index_start) => {
	...
	console.log(prices, carbon, carbonprice, index_start);
...
const calculateC = (start, end, prices) => {
	// added by asr 8-14-09; modified by asr 6-5-11
...
const solveForAnnualAverageRate = (computedC, duration) => {
	// added by asr 8-15-09; modified by asr 6-5-11 to use start date's index < end date's index and
...
export default function EercForm() {
	...
	let pecsTotal =
		parseFloat(pecs.coal) +
		parseFloat(pecs.distillateoil) +
		parseFloat(pecs.electricity) +
		parseFloat(pecs.naturalgas) +
		parseFloat(pecs.residual);
	...
	const handlePecsChange = (prop) => (event) => {
		let v = event.target.value
			.replace(nonnumeric_re, "")
			.replace(clean_re, "$1")
			.replace(trim0_re, "$<neg>$<num>")
			.replace(needlead0_re, "$<neg>0.")
			.replace(empty_re, "0");
		event.target.value = v;
		setPecs({ ...pecs, [prop]: v });
	};
	...
	const handleInflationrateChange = (event) => {
		let v = event.target.value
			.replace(nonpercent_re, "")
			.replace(pctclean_re, "$1")
			.replace(trim0_re, "$<neg>$<num>")
			.replace(needlead0_re, "$<neg>0.")
			.replace(empty_re, "0");
		event.target.value = v;
	...
	const calculateCarbonPrice = useCallback(
		(CO2Factor, cP, isElectricity, baseyear) => {
	...
	useEffect(() => {
		...
		const v =
			...
			carbonprice !== "" &&
			Object.keys(carbonprices).includes(carbonprice) &&
	...
	const handlePDF = (e) => {
		...
		pdf
			...
			.text("NIST Energy Escalation Rate Calculation", center, vc, "center");
		pdf
			.setTextColor(0, 0, 255)
			.setFontSize(12)
			.setFont("courier", "normal")
			.text("https://pages.nist.gov/eerc/", center, vc + 0.3, "center");
		vc += 1.25;
		pdf
			.setTextColor(0, 0, 0)
			.setFontSize(20)
			.setFont("helvetica", "bold")
			.text("Input Parameters:", lm + 0.5, vc);
		...
		for (const et of energytypes) {
			if (pecs[et.slug] > 0) {
				pdf
					.setTextColor(0, 0, 0)
					.setFontSize(14)
					.text(`Percent from ${et.name}:`, lm + 1, vc);
				pdf
					.setTextColor(0, 0, 255)
					.setFontSize(14)
					.text(`${pecs[et.slug]}%`, lm + 4, vc);
				vc += 0.2;
			}
		}
		vc += 0.2;
		pdf
			.setTextColor(0, 0, 0)
			.setFontSize(14)
			.text("Location:", lm + 1, vc);
		pdf
			.setTextColor(0, 0, 255)
			.setFontSize(14)
			.text(`${locale} (${stateToRegion(/*ZipToState[locale]*/ locale)})`, lm + 4, vc);
		vc += 0.25;
		pdf
			.setTextColor(0, 0, 0)
			.setFontSize(14)
		...
		pdf
			.setTextColor(0, 0, 0)
			.setFontSize(14)
			.text("Contract Start:", lm + 1, vc);
		pdf
			.setTextColor(0, 0, 255)
			.setFontSize(14)
			.text(startdate.toString(), lm + 4, vc);
		vc += 0.25;
		pdf
			.setTextColor(0, 0, 0)
			.setFontSize(14)
			.text("Contract Duration:", lm + 1, vc);
		pdf
			.setTextColor(0, 0, 255)
			.setFontSize(14)
			.text(`${duration.toString()} years`, lm + 4, vc);
		vc += 0.25;
		pdf
			.setTextColor(0, 0, 0)
			.setFontSize(14)
			.text("Social Cost of GHG Assumptions:", lm + 1, vc);
		pdf
			.setTextColor(0, 0, 255)
			.setFontSize(14)
			.text(carbonprice, lm + 4, vc);
		vc += 0.25;
		pdf
			.setTextColor(0, 0, 0)
			.setFontSize(14)
			.text("Annual Inflation Rate:", lm + 1, vc);
		pdf
			.setTextColor(0, 0, 255)
			.setFontSize(14)
			.text(`${parseFloat(inflationrate).toFixed(2)}%`, lm + 4, vc);
		vc += 1;
		pdf
			.setTextColor(0, 0, 0)
			.setFontSize(20)
			.setFont("helvetica", "bold")
			.text("Results:", lm + 0.5, vc);
		vc += 0.4;
		pdf
			.setTextColor(0, 0, 0)
			.setFontSize(16)
			.text("Real Rate:", lm + 1, vc);
		pdf
			.setTextColor(96, 0, 172)
			.setFontSize(16)
			.text(`${isNaN(result_real) ? "---" : parseFloat(result_real).toFixed(2)}%`, lm + 4, vc);
		vc += 0.4;
		pdf
			.setTextColor(0, 0, 0)
			.setFontSize(16)
			.text("Nominal Rate:", lm + 1, vc);
		pdf
			.setTextColor(96, 0, 172)
			.setFontSize(16)
			.text(`${isNaN(result_nominal) ? "---" : parseFloat(result_nominal).toFixed(2)}%`, lm + 4, vc);
		vc += 0.4;
		pdf
			.setTextColor(0, 0, 0)
			.setFontSize(16)
			.text("Calculated on:", lm + 1, vc);
		pdf
			.setTextColor(0, 0, 0)
			.setFontSize(12)
			.setFont("helvetica", "normal")
			.text(now.toLocaleString(), lm + 4, vc);
		vc += 0.4;
		pdf
			.setTextColor(0, 0, 0)
			.setFontSize(16)
			.setFont("helvetica", "bold")
			.text("Datafile version:", lm + 1, vc);
		pdf
			.setTextColor(0, 0, 0)
			.setFontSize(14)
			.setFont("helvetica", "normal")
			.text(CO2ePrices.startyear.toString(), lm + 4, vc);
	...
	if (typeof CO2ePrices.startyear === "undefined" || CO2ePrices.startyear === "null") {
		dataset_msg = (
			<Typography variant="h6" color="error" gutterBottom>
				[Warning: Dataset is not loaded!]
			</Typography>
		);
	} else {
		dataset_msg = (
			<Typography variant="body2" gutterBottom>
				(Loaded {CO2ePrices.startyear} dataset)
			</Typography>
		);
	}
	...
	let warnings_msg = warnings.map((m, i) => {
		return (
			<Alert key={`alert${i}`} severity="warning">
				{m}
			</Alert>
		);
	});
	...
	return (
		<form className={classes.root} noValidate autoComplete="off">
			...
			<Typography variant="body2" paragraph>
				...
				<br />
				The EERC User Guide is here:&nbsp;&nbsp;
				<Link target="_blank" rel="noopener" href="EERC User Guide.html">
					HTML
				</Link>
				&nbsp;&nbsp;
				<Link target="_blank" rel="noopener" href="EERC User Guide.pdf">
					PDF
				</Link>
			</Typography>
					...
					{energytypes.map((energy, index) => (
						<Grid item xs={4} sm={2} key={"grid" + index}>
							<HtmlTooltip
								...
								title={
									<React.Fragment>
										{"Percentage of energy cost savings in dollars that is attributable to " +
											energy.name.toLowerCase() +
											" used in the project. This input is used to weight the escalation rate."}
									</React.Fragment>
								}
							>
								<TextField
            ...
            /> */}
						<MySelect
							...
							tooltip=<React.Fragment>
								Selecting the state in which the project is located is needed to select the associated energy price
								escalation rates (by census region) and CO2 pricing and emission rates (currently by state).
							</React.Fragment>
						/>
					</Grid>
					<Grid item xs={6} sm={3}>
						<MySelect
							...
							tooltip=<React.Fragment>
								Selection of commercial sector or industrial sector determines the escalation rate schedule applied to
			...
			<fieldset>
				...
				<Grid container alignItems="baseline" justifyContent="center" spacing={6}>
					...
					<Grid item xs={6} sm={3}>
						<HtmlTooltip arrow title={<React.Fragment>{"Number of years of the contract term"}</React.Fragment>}>
						...
						<MySelect
							...
							tooltip={
								<React.Fragment>
									...
									<ul>
										<li>
											<em>{"No Carbon Price"}</em> {"assumes that no carbon policy is enacted (status quo)"}
										</li>
										<li>
											<em>{"Low - $20 in 2024 - 5% DR (average)"}</em>{" "}
											{"= average social cost of GHG assuming a 5% real discount rate"}
										</li>
										<li>
											<em>{"Medium - $66 in 2024 - 3% DR (average)"}</em>{" "}
											{
												"= average social cost of GHG assuming a 3% real discount rate. Best match to DOE and OMB real discount rates."
											}
										</li>
										<li>
											<em>{"High - $198 in 2024 - 3% DR (95th percentile)"}</em>{" "}
											{"= 95th Percentile social cost of GHG assuming a 3% real discount rate"}
										</li>
									</ul>
								</React.Fragment>
							}
			...
			{warnings_msg.length ? (
				<div>
					<fieldset style={{ border: "6px groove", borderColor: "red" }}>{warnings_msg}</fieldset>
					<br />
				</div>
			) : null}
			<fieldset style={{ border: "6px groove", borderColor: "black" }}>
				...
				<Grid
					...
					style={{ backgroundColor: isNaN(result_real) ? "lightgrey" : "#efd" }}
				>
